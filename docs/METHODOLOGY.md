...
4. **Review** stories against **S7** briefs (`@requirement-risk`, `@ticket-splitter`, `@ticket-test-design`).
...
**S7** `*-domain-brief.md` files are the reader-facing deliverables. `@requirement-risk`, `@ticket-splitter`, and `@ticket-test-design` consume **S7**, not raw wiki pages.
...
- **`@ticket-test-design`** — release/Done proof cases (must/should/later; Contract readiness vs Pack note).

Rules: [`.cursor/rules/requirement_risk.md`](../.cursor/rules/requirement_risk.md) · [`.cursor/rules/ticket_system.md`](../.cursor/rules/ticket_system.md) · [`.cursor/rules/ticket_test_design.md`](../.cursor/rules/ticket_test_design.md).
