...
4c. **Compose exhaustiveness**: after S3, run `tagging_acceptance.py --after-s3`. After S7, run `--after-s7` (optionally `--strict`). Under-write (few rules vs many `pages_with_props`) → land leftover evidence or Open items; no “fully covered” claim. Low-evidence S7 requires an **Evidence insufficiency** banner (non-SSOT for risk).
4d. **Zero-rule fake coverage banned**: confirmed slug with tagged sources but **zero** `### Rule` headings in S7 → revert to **pending** or rewrite. Do not ship empty core-rules as committed briefs.
4e. **Axes stay; layout is not a domain cut**: land sources into existing strategy axes (or a human-confirmed new slug). Do not promote wiki/Jira filing layout (app/channel/facet) into default modules, and do **not** hard-code tenant product remount tables into the pack — see [`industry-axis-remount.md`](./industry-axis-remount.md).
