...
4c. **Compose exhaustiveness**: after S3, run `tagging_acceptance.py --after-s3`. After S7, run `--after-s7` (optionally `--strict`). Under-write (few rules vs many `pages_with_props`) → remount or Open items; no “fully covered” claim. Low-evidence S7 requires an **Evidence insufficiency** banner (non-SSOT for risk).
...
4e. **Keep industry axes; remount product surfaces**: do not recreate Mall/Hui/Gateway/Messaging modules by default. Remount dense app/channel pages into existing axes per [`industry-axis-remount.md`](./industry-axis-remount.md). Propose a new slug only when no axis fits and the human confirms.
